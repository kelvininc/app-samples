...
class Store:
    ...
    def _setup(self) -> None:
        with self._lock:                 # same lock discipline as every other _con op
            ...
            con = self._con = duckdb.connect(self.db_path)
            con.execute("CREATE SEQUENCE IF NOT EXISTS seq START 1")
            con.execute(
                """
    ...
    def _read(self, limit: int) -> Records:
        with self._lock:
            ...
            total = self._total()
        data = [{"timestamp": ts, "asset": asset, "datastream": ds, "payload": payload}
                for _seq, ts, asset, ds, payload in rows]
