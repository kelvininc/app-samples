...
class TestDrainLoop:
    ...
    async def test_survives_store_drop_failure_and_retries(self) -> None:
        """A store.drop failure inside a tick (e.g. transient DuckDB disk error) is logged
        and retried next interval instead of killing the drain task permanently."""
        ticks = {"n": 0}

        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], 7, 1, 0)               # cursor set -> the tick calls store.drop
            async def teardown(self) -> None: ...
        ...
        clock = YieldClock()
        task = asyncio.create_task(
            drain.drain(OkWriter(), ExplodingStore(), Upload(interval=60), Buffer(), clock)
        )
        while ticks["n"] < 3:                          # loop survives repeated drop failures
            await asyncio.sleep(0)
        task.cancel()
        with pytest.raises(asyncio.CancelledError):    # cancellation still propagates
            await task
        assert ticks["n"] >= 3
        assert 60 in clock.slept                       # failure path waits one upload interval
