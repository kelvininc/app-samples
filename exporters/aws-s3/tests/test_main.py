    ...
    return (
        ...
        .set_configuration({
            "s3": {"region": "us-east-1", "bucket": "telemetry",
                   "auth": {"access_key_id": "AKIA", "secret_access_key": "shhh"}},
...
async def test_three_types_buffer_and_drain(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "S3Writer", lambda cfg, fmt: fake)   # main builds S3Writer(s3, upload.format)
...
async def test_on_disconnect_tears_down_writer_and_store(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "S3Writer", lambda cfg, fmt: fake)
...
async def test_invalid_configuration_raises(tmp_path: Path) -> None:
    """A config with only one S3 key fails on_connect loudly instead of starting half-built."""
    _reset(tmp_path)
    bad = (
        ManifestBuilder.from_app_yaml()
        .add_asset("pump-1")
        ...
        .set_configuration({
            "s3": {"region": "us-east-1", "bucket": "telemetry",
                   "auth": {"access_key_id": "AKIA"}},   # missing secret_access_key -> validator fails
        })
        .build()
    )
