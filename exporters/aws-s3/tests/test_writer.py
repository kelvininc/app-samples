...
class TestSetup:
    ...
    @pytest.mark.parametrize("code,status", [("403", 403), ("AccessDenied", 403), ("404", 404), ("NoSuchBucket", 404)])
    async def test_config_error_fails_fast(self, monkeypatch: pytest.MonkeyPatch, code: str, status: int) -> None:
        """A deterministic misconfiguration (bad credentials or missing bucket) raises."""
