...
# HTTP statuses that mean the deployment is misconfigured (never self-heal): a bucket in a
# different region (301/PermanentRedirect from head_bucket), bad/insufficient credentials
# (401/403), or a bucket that doesn't exist (404) - retrying these never recovers, so fail
# fast at setup instead of buffering forever. This tuple is the single source of the numeric
# statuses; the string Code set below names the same failures as boto3 reports them.
_CONFIG_ERROR_STATUSES = (301, 401, 403, 404)
_CONFIG_ERROR_CODES = {"301", "PermanentRedirect", "403", "AccessDenied", "404",
...
class S3Writer:
    ...
    def _validate(self) -> None:
        ...
        # keys, 301/PermanentRedirect region mismatch) raise, so the deployment fails fast and
        # visibly. Transient failures (endpoint unreachable, timeouts, 5xx) and anything
        # unclassified only warn: the persistent buffer keeps accepting data and the drain's
        # retry owns connectivity.
        try:
            ...
            if code in _CONFIG_ERROR_CODES or status in _CONFIG_ERROR_STATUSES:
