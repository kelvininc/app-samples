...
class TestDrainLoop:
    ...
    async def test_drain_survives_drop_failure(self) -> None:
        """A failing store.drop (e.g. disk full) doesn't kill drain(): the tick error is
        logged, the loop sleeps one interval, and the next tick runs."""
        ticks = {"n": 0}

        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], 1, 1, 0)               # non-empty -> _tick calls store.drop
            async def teardown(self) -> None: ...
        ...
        with pytest.raises(asyncio.CancelledError):
            await task
        assert ticks["n"] >= 3                          # loop kept ticking past the failures
        assert 7 in clock.slept                         # failed tick slept one interval before retrying
