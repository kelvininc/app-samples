...
async def test_three_types_buffer_and_drain(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "ADLSWriter", lambda cfg, fmt: fake)   # main builds ADLSWriter(adls, upload.format)
...
async def test_on_disconnect_tears_down_writer_and_store(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "ADLSWriter", lambda cfg, fmt: fake)
...
async def test_invalid_configuration_raises(tmp_path: Path) -> None:
    """A config missing the container fails on_connect loudly instead of starting half-built."""
    _reset(tmp_path)
    bad = (
        ManifestBuilder.from_app_yaml()
        .add_asset("pump-1")
        .add_input("temperature", "number")
        .set_configuration({
            "adls": {"account_name": "telemetrylake"},   # missing container -> validation fails
        })
        .build()
    )
