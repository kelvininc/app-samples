...
class TestUploadAndBuffer:
    ...
    def test_defaults(self) -> None:
        ...
        assert s.buffer.max_backlog == 1_000_000     # bounded by default; 0 is an explicit opt-in

    def test_max_backlog_zero_is_explicit_unbounded_opt_in(self) -> None:
        """max_backlog=0 is still accepted as the explicit unbounded setting."""
    ...
    def test_rejects_out_of_bounds(self, bad: dict) -> None:
        """Upload knobs enforce their bounds (interval ge=1: 0 would busy-spin the drain;
        batch_size le= cap: the drain reads a whole batch into RAM, so an unbounded value
        would mean an unbounded per-upload buffer)."""
        with pytest.raises(ValidationError):
            Settings(**FULL, upload=bad)

    def test_accepts_batch_size_at_cap(self) -> None:
        """The cap value itself is accepted."""
        assert Settings(**FULL, upload={"batch_size": 100_000}).upload.batch_size == 100_000
