...
class TestDrainLoop:
    ...
    async def test_loop_survives_a_failing_tick(self) -> None:
        """An exception escaping _tick (here: store.drop failing) is logged and retried
        next interval instead of killing the drain task; cancellation still propagates."""
        ticks = {"n": 0}

        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], 1, 0, 0)               # cursor set -> _tick calls store.drop
            async def teardown(self) -> None: ...
        ...
        assert 7 in clock.slept                        # failure path slept one interval
