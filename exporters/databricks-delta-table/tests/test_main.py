...
def _manifest():
    ...
    return (
        ...
        .set_configuration({
            "databricks": {"server_hostname": "h", "http_path": "/p", "delta_table": "c.s.t",
...
async def test_three_types_buffer_and_drain(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "DeltaTableWriter", lambda cfg: fake)   # main builds DeltaTableWriter(settings.databricks)
...
async def test_on_disconnect_tears_down_writer_and_store(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "DeltaTableWriter", lambda cfg: fake)
...
async def test_invalid_configuration_raises(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """A config missing the auth credential fails on_connect loudly instead of starting half-built."""
    _reset(tmp_path)
    bad = (
        ManifestBuilder.from_app_yaml()
        .add_asset("pump-1")
        ...
        .set_configuration({
            "databricks": {"server_hostname": "h", "http_path": "/p", "delta_table": "c.s.t",
                           "auth": {"method": "access_token"}},   # no access_token -> one-auth fails
        })
        .build()
    )
