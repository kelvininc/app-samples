...
class TestAppend:
    ...
    async def test_append_normalizes_aware_timestamps_to_naive_utc(self, store: Store) -> None:
        """A tz-aware timestamp is stored as its naive UTC instant, so the buffered value
        doesn't depend on DuckDB's session timezone."""
        ts = datetime(2026, 1, 1, 12, 0, tzinfo=timezone(timedelta(hours=2)))
        await store.append(ts, "pump-1", "temperature", 1.0)
        got = (await store.read(10)).rows[0]["timestamp"]
        assert got == datetime(2026, 1, 1, 10, 0) and got.tzinfo is None
