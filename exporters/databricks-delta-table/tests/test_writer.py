...
from writer import DeltaTableWriter, OAuthCredentialError, _fatal_setup_error


def _oauth_cfg():
    """A minimal valid Databricks config on the OAuth service-principal path."""
...
class TestFatalErrorClassification:
    """_fatal_setup_error classifies by structured connector/SDK error types, never by
    substring-matching the message."""

    @pytest.mark.parametrize("exc", [
        ServerOperationError("TABLE_OR_VIEW_NOT_FOUND"),
        RequestError("refused", context={"http-code": 401}),
        RequestError("refused", context={"http-code": 403}),
        OAuthCredentialError("invalid_client: ..."),          # wrapped OAuth token-fetch failure
    ])
    def test_fatal(self, exc: Exception) -> None:
        assert _fatal_setup_error(exc) is True

    @pytest.mark.parametrize("exc", [
        RequestError("unavailable", context={"http-code": 503}),
        RequestError("no code at all"),                       # context has no http-code
        OSError("connection timed out"),
        RuntimeError("dropped 403 rows"),                     # bare "403" in text must not misfire
        TimeoutError(),                                       # str(e) is empty
        ValueError("invalid_client: ..."),                    # a bare ValueError isn't fatal unless wrapped
    ])
    def test_not_fatal(self, exc: Exception) -> None:
        assert _fatal_setup_error(exc) is False
...
class TestSetupProbe:
    ...
    async def test_bad_oauth_secret_fails_fast(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """A rejected OAuth client secret surfaces as a bare ValueError ("invalid_client: ...")
        from the databricks-sdk M2M token fetch during connect. _connection wraps it as
        OAuthCredentialError, so setup() fails fast (raises) instead of warn-and-retrying a
        credential that will never start working."""
        w = DeltaTableWriter(_oauth_cfg())
        monkeypatch.setattr(writer_mod, "Config", lambda **kw: "conf")
        monkeypatch.setattr(writer_mod, "oauth_service_principal", lambda conf: object())

        class FakeSql:
            def connect(self, **kw):
                raise ValueError("invalid_client: The client credential is invalid")

        monkeypatch.setattr(writer_mod, "sql", FakeSql())
        with pytest.raises(OAuthCredentialError, match="invalid_client"):
            await w.setup()
