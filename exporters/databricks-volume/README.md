...
Within a single buffered batch, writes are **last-write-wins per key**: if two values arrive
with the same `(timestamp, asset, datastream)` before the batch is uploaded, the buffer keeps
only the most recent one. A corrected value that lands after its key was already uploaded
re-queues under a fresh sequence and uploads again, so the Delta table can hold both the
original and the correction (resolve by taking the latest per key downstream).
