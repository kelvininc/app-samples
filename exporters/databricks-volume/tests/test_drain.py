...
class TestDrainLoop:
    ...
    async def test_drain_survives_drop_failure(self) -> None:
        """A store.drop failure inside a tick doesn't kill drain(): the loop logs, sleeps
        one interval, and retries; only cancellation stops it."""
        drops = {"n": 0}

        class DropFailStore(_FakeStore):
            async def drop(self, cursor: int) -> None:
                drops["n"] += 1
        ...
        task = asyncio.create_task(
            drain.drain(OkWriter(), DropFailStore(), Upload(interval=5), Buffer(), YieldClock())
        )
        while drops["n"] < 3:
            await asyncio.sleep(0)
        task.cancel()
