...
# conftest.py stubs the zerobus SDK modules into sys.modules when the real wheel isn't
# installed, so main imports cleanly here; ZerobusWriter is monkeypatched out below either way.
import main
...
def _manifest():
    ...
    return (
        ...
        .set_configuration({
            "databricks": {"server_hostname": "h", "zerobus_endpoint": "ep", "delta_table": "c.s.t",
...
async def test_three_types_buffer_and_drain(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "ZerobusWriter", lambda cfg: fake)   # main builds ZerobusWriter(settings.databricks)
...
async def test_on_disconnect_tears_down_writer_and_store(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "ZerobusWriter", lambda cfg: fake)
...
async def test_invalid_configuration_raises(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """A config missing client_secret fails on_connect loudly instead of starting half-built."""
    _reset(tmp_path)
    bad = (
        ManifestBuilder.from_app_yaml()
        .add_asset("pump-1")
        ...
        .set_configuration({
            "databricks": {"server_hostname": "h", "zerobus_endpoint": "ep", "delta_table": "c.s.t",
                           "auth": {"client_id": "cid"}},   # no client_secret -> require-both fails
        })
        .build()
    )
