...
class TestUploadAndBuffer:
    ...
    def test_defaults(self) -> None:
        ...
        assert s.buffer.max_backlog == 1_000_000     # bounded by default; 0 is explicit opt-in

    def test_max_backlog_zero_is_explicit_unbounded(self) -> None:
        """A caller can still opt into an unbounded buffer with an explicit 0."""
        assert Settings(**OAUTH, buffer={"max_backlog": 0}).buffer.max_backlog == 0
