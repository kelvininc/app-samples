...
class _FakeStream:
    """Records ingested records and flush/close calls; optionally blows up on flush."""
    ...
    async def ingest_record_offset(self, record: dict) -> None:
...
class TestStreamLifecycle:
    ...
    async def test_batch_ingests_flushes_and_unwraps_credentials(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """A batch reuses the setup-opened stream, ingests each record, flushes once, with creds unwrapped."""
        stream = _FakeStream()
    ...
    async def test_empty_batch_ingests_nothing(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert r.n_rows == 0 and stream.ingested == []
        assert len(holder["sdk"].create_calls) == 1     # setup's open, nothing more

    async def test_failed_flush_closes_stream_and_reraises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """A flush failure drops the stream (so the next attempt reopens) and re-raises."""
        stream = _FakeStream(fail_flush=True)
