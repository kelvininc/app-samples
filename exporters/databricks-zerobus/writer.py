...
class ZerobusWriter:
    ...
    Recovery is at-least-once, not exactly-once: flush blocks until the server acks the
    whole batch (so the buffer is only trimmed once data is durable), but a committed-but-
    unacked batch is re-sent on the next tick and, since the table enforces no key,
    duplicates; downstream should tolerate that.
    """
    ...
    def build_record(row: dict) -> dict:
        ...
        JSON, which can't carry a datetime), and a non-finite float payload becomes None.
        """
        return {
    ...
    async def write_batch(self, store: Store, limit: int) -> Records:
        ...
        if r.cursor is not None:
            ...
            try:
                for row in r.rows:
                    # ingest_record_offset + flush (not nowait): flush blocks until the server
                    # acks the whole batch, so the buffer is only trimmed once data is durable.
                    await stream.ingest_record_offset(self.build_record(row))
                await stream.flush()
            except Exception:
