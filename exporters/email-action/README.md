...
- **Malformed payloads** (missing/empty `to`, `subject`, or `message.text`) and **send failures**
  (SMTP errors, network errors, invalid headers) each publish a failure ack (`CustomActionResult`
  with `success: false` and the reason) back to the platform.
