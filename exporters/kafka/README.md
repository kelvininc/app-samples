...
- **Value** (JSON): `{"timestamp": "2026-01-01T12:00:00+00:00", "asset": "pump-1", "datastream": "temperature", "payload": 42.5}`. The timestamp is ISO-8601 in UTC (an explicit `+00:00` offset, so a consumer never reads it as local time); a non-finite float payload (NaN/Inf) is coerced to `null`, since literal NaN/Infinity is invalid JSON.
...
A record the producer can never send (a payload larger than `max_request_size`, or one the broker rejects with `RecordTooLargeError`) is **dead-lettered**: it's logged with a warning that identifies the asset, datastream, and timestamp, then dropped so it can't wedge the head of the buffer. Transient failures (broker unreachable, timeouts) are not dropped; the whole batch stays buffered and retries on the next interval.
...
The buffer keys rows on `(timestamp, asset, datastream)`, so **last write wins**: if two values arrive for the same asset, datastream, and timestamp before the batch is uploaded, the buffer keeps only the most recent one and the earlier value is never published.
...
### Integration Tests
```bash
pip install 'testcontainers[kafka]'              # real-server deps
pytest -m integration                            # smoke test against a live Kafka broker (Docker required)
```

`test_integration.py` boots a Kafka container and drives the real `KafkaWriter`: it produces a
buffered batch over aiokafka, then consumes the topic back and asserts each JSON message carries
the right asset/datastream/payload and a UTC-marked timestamp. The suite is marker-gated,
deselected by default, and requires Docker.
