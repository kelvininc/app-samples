...
class TestTick:
    ...
    async def test_successful_send_drops_the_batch_seqs_and_caps(self) -> None:
        """A successful write drops exactly the returned seq set and runs the cap."""
        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                return Records([], list(range(1, 101)), 100, 0)   # count == batch_size
            async def teardown(self) -> None: ...

        store = _FakeStore()
        await drain._tick(OkWriter(), store, Upload(batch_size=100), Buffer(), RecordingClock())
        assert store.drops == [list(range(1, 101))] and store.caps == [0]

    async def test_empty_batch_never_calls_drop(self) -> None:
        """An empty read (no seqs) must not reach store.drop: nothing was sent."""
        class EmptyWriter:
            async def write_batch(self, store, limit) -> Records:
                return Records([], [], 0, 0)
            async def teardown(self) -> None: ...
        ...
        store = _FakeStore()
        await drain._tick(EmptyWriter(), store, Upload(batch_size=100), Buffer(), RecordingClock())
        assert store.drops == []
    ...
    async def test_full_batch_skips_sleep(self) -> None:
        ...
        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                return Records([], list(range(1, 101)), 100, 0)   # == batch_size
            async def teardown(self) -> None: ...
    ...
    async def test_partial_batch_sleeps_one_interval(self) -> None:
        ...
        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                return Records([], list(range(1, 11)), 10, 0)     # 10 < batch_size 100
            async def teardown(self) -> None: ...
...
class TestDrainLoop:
    ...
    async def test_loops_until_cancelled_without_tearing_down(self) -> None:
        ...
        class CountingWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], [], 0, 0)              # empty -> no drop, loop sleeps each tick
            async def teardown(self) -> None:
                teardowns["n"] += 1
    ...
    async def test_survives_store_failure_inside_tick(self) -> None:
        ...
        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], list(range(1, 11)), 10, 0)   # success -> _tick calls store.drop(seqs)
            async def teardown(self) -> None: ...
