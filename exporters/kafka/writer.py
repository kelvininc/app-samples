...
# Per-record permanent errors: the record itself is un-producible (payload larger than the
# producer's max_request_size or the broker's max.message.bytes). Retrying it never succeeds, and
# because reads are strict FIFO by seq, one such record at the head would wedge the whole buffer.
# We dead-letter it (drop with a warning) so the buffer advances; transient / whole-broker errors
# stay on the retry path (raise, whole batch re-sent next tick).
_POISON_ERRORS = (MessageSizeTooLargeError, RecordTooLargeError)
...
class KafkaWriter:
    ...
    def build_record(row: dict) -> bytes:
        ...
        The Store keeps timestamps as naive UTC, so mark them tz-aware UTC before formatting
        (emitting a "+00:00" offset) instead of a bare wall-clock string a consumer could read
        as local time. A non-finite float payload becomes null.
        """
        return json.dumps({
    ...
    @staticmethod
    def _log_poison(row: dict, topic: str, err: BaseException) -> None:
        logger.warning("Dropping un-producible record (dead-letter); it will not be published",
                       asset=row["asset"], datastream=row["datastream"],
                       timestamp=row["timestamp"].isoformat(), topic=topic,
    ...
    async def write_batch(self, store: Store, limit: int) -> Records:
        ...
        if r.cursor is not None:
            producer = await self._ensure_producer()
            skipped = 0                     # rows whose stream lost its topic mapping (redeploy)
            poisoned = 0                    # rows dead-lettered as permanently un-producible
            non_finite = 0
            batch_topics: set[str] = set()
            pending: list[tuple] = []       # (future, row, topic) for rows actually sent
            try:
                for row in r.rows:
                    ...
                    if topic is None:
                        ...
                        continue
                    try:
                        # send() serializes eagerly and raises MessageSizeTooLargeError here when
                        # the payload exceeds max_request_size: a poison record, caught per-row.
                        future = await producer.send(topic, value=self.build_record(row),
                                                     key=self.build_key(row))
                    except _POISON_ERRORS as e:
                        poisoned += 1
                        self._log_poison(row, topic, e)
                        continue
                    ...
                    pending.append((future, row, topic))
                # Await deliveries individually (return_exceptions) so one record the broker
                # permanently rejects is dead-lettered while a transient failure still fails the
                # whole batch. acks=all: the buffer is trimmed only once every record acked.
                results = await asyncio.gather(*(f for f, _, _ in pending), return_exceptions=True)
                transient: Optional[BaseException] = None
                for (_future, row, topic), res in zip(pending, results):
                    if isinstance(res, _POISON_ERRORS):
                        poisoned += 1
                        self._log_poison(row, topic, res)
                    elif isinstance(res, BaseException):
                        transient = transient or res    # keep the first; re-raise for a whole-batch retry
            ...
            if poisoned:
                extra["dead_lettered"] = poisoned
            logger.info("Published to Kafka", rows=r.n_rows - skipped - poisoned, backlog=r.backlog,
