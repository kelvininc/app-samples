...
## Failure Behavior
- **Invalid configuration** (missing/blank webhook URL, a URL that is not `http(s)`, or an
  unresolved secret) terminates the app at connect with exit code 1; it never starts
  half-configured. The rejection reason never includes the URL, which embeds a token.
- **Malformed payload** (e.g. `message` is not an object, or `text` is missing, empty, or
  whitespace-only) acks the action with `success: false` and a message describing the invalid
  fields.
