...
class ExporterApplication:
    ...
    async def on_custom_action(self, action: CustomAction) -> None:
        if action.type.lower() != "teams message":
            logger.warning("Received unexpected Custom Action", action=action)
            return

        logger.info("Received Teams Message Action", action=action)
        ...
        try:
            settings = Settings(**self.app.app_configuration)
        except ValidationError as e:
            logger.error(
                "Teams webhooks are not configured",
                action=action,
                errors=e.errors(include_url=False, include_input=False),
            )

            result = action.result(success=False, message="Teams webhooks are not configured")
            await self.app.publish(result)

            return

        # Call integration
        integration = TeamsIntegration(settings=settings)

        response = await integration.send_teams_message(channel_name=channel, message=message)

        # Publish action result
        result = action.result(success=response.success, message=response.message)
        await self.app.publish(result)

        logger.info("Finished handling Teams Message Action", action=action, success=response.success)
