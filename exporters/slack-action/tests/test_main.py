...
@pytest.mark.parametrize("payload", [
    {"channel": "   ", "message": {"text": "hi"}},
    {"channel": "#alerts", "message": {"text": "   "}},
])
async def test_whitespace_only_fields_rejected(payload: dict) -> None:
    """Whitespace is stripped before min_length, so a blank channel or text fails validation."""
    with pytest.raises(ValidationError):
        SlackPayload.model_validate(payload)
