...
def test_blank_token_fails_fast(token: str) -> None:
    """A blank token would only fail on the first Slack API call; fail at config time instead."""
    with pytest.raises(ValidationError):
        Settings(slack={"token": token})
