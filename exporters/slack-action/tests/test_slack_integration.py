...
class _FakeClient:
    ...
    def __init__(
        ...
        # Mirrors AsyncWebClient.retry_handlers so SlackIntegration.__init__ can append its own.
...
async def test_rate_limit_retry_handler_registered() -> None:
    """A 429 retry handler is appended alongside the client's default connection-error handler.

    Builds a genuine SlackIntegration against a real AsyncWebClient (construction is pure, no
    I/O) and asserts both handlers survive: the SDK's actual default AsyncConnectionErrorRetryHandler
    and the appended AsyncRateLimitErrorRetryHandler. A regression to
    retry_handlers=[AsyncRateLimitErrorRetryHandler()] would drop the default handler and fail here.
    """
    integration = SlackIntegration(Slack(token="xoxb-test"))
    assert any(isinstance(h, AsyncConnectionErrorRetryHandler) for h in integration.client.retry_handlers)
    assert any(isinstance(h, AsyncRateLimitErrorRetryHandler) for h in integration.client.retry_handlers)
