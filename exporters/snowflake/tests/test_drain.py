...
class TestAttempt:
    ...
    async def test_warning_logs_carry_attempt_context(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Every failed attempt logs a warning: intermediate ones say 'retrying' with the
        backoff delay, the final one gives up without a retry_in. error_type identifies
        the failure even when str(e) is empty (TimeoutError, ConnectionError, ...)."""
        warnings: list[dict] = []
        monkeypatch.setattr(drain.logger, "warning", lambda msg, **kw: warnings.append({"msg": msg, **kw}))

        async def op() -> Records:
            raise TimeoutError()                 # str() is empty: error_type must still say what broke

        assert await drain._attempt(op, Retry(attempts=3, base_delay=1, max_delay=30),
                                    RecordingClock()) is None
        assert warnings == [
            {"msg": "Upload attempt failed, retrying", "attempt": "1/3", "retry_in": 1,
             "error": "", "error_type": "TimeoutError"},
            {"msg": "Upload attempt failed, retrying", "attempt": "2/3", "retry_in": 2,
             "error": "", "error_type": "TimeoutError"},
            {"msg": "Upload attempt failed", "attempt": "3/3",
             "error": "", "error_type": "TimeoutError"},
        ]
...
class TestTick:
    ...
    async def test_exhausted_retries_log_give_up_with_backlog(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """When every attempt fails, the tick returns None and logs the give-up summary with
        the configured attempts, the running failure streak, and the buffered backlog; a
        failed tick still sleeps one interval (it never fast-loops)."""
        errors: list[dict] = []
        monkeypatch.setattr(drain.logger, "error", lambda msg, **kw: errors.append({"msg": msg, **kw}))
        ...
        class BackloggedStore(_FakeStore):
            async def count(self) -> int:
                return 17

        clock = RecordingClock()
        r = await drain._tick(FailingWriter(), BackloggedStore(),
                              Upload(interval=60, retry=Retry(attempts=2, base_delay=0)),
                              Buffer(), clock, failed_ticks=4)
        assert r is None
        assert errors == [{"msg": "Upload failed; batch stays buffered until next interval",
                           "attempts": 2, "consecutive_failures": 5, "backlog": 17}]
        assert clock.slept == [0, 60]           # one zero backoff between attempts, then the interval
...
class TestDrainLoop:
    ...
    async def test_survives_store_failure_inside_tick(self) -> None:
        """A store.drop/cap failure inside a tick is logged and retried next interval;
        it must not kill the drain task (on_data would keep buffering with no uploads)."""
        ticks = {"n": 0}

        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], 42, 10, 0)             # success -> _tick calls store.drop(42)
            async def teardown(self) -> None: ...

        class BrokenStore(_FakeStore):
            async def drop(self, cursor: int) -> None:
                raise RuntimeError("disk full")        # unguarded before; killed the loop

        class YieldClock(RecordingClock):
            async def sleep(self, delay: float) -> None:
                self.slept.append(delay)
                await asyncio.sleep(0)                 # yield so the task stays cancellable
        ...
        clock = YieldClock()
        task = asyncio.create_task(
            drain.drain(OkWriter(), BrokenStore(), Upload(interval=7), Buffer(), clock)
        )
        while ticks["n"] < 3:
            await asyncio.sleep(0)
        assert not task.done(), "drain task died on a store failure"
        task.cancel()
        with pytest.raises(asyncio.CancelledError):
            await task
        assert ticks["n"] >= 3                          # kept ticking past the failures
        assert clock.slept and set(clock.slept) == {7}  # backed off one interval per failed tick

    async def test_survives_store_cap_failure_inside_tick(self) -> None:
        """A store.cap failure is absorbed like any other tick error, even on an empty batch
        (no cursor, so drop never runs): the disk-safety guard fires every tick, so its
        failure must not kill the loop either."""
        caps = {"n": 0}

        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                return Records([], None, 0, 0)             # empty batch: cap is the only failure
            async def teardown(self) -> None: ...
        ...
        with pytest.raises(asyncio.CancelledError):
            await task
        assert caps["n"] >= 3                           # kept ticking past the failures
        assert clock.slept and set(clock.slept) == {7}  # backed off one interval per failed tick
