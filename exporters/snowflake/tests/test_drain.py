...
class TestDrainLoop:
    ...
    async def test_tick_failure_is_absorbed_and_loop_retries(self) -> None:
        """A tick that raises past the retry helper (here: store.drop failing after a
        successful send) is logged, sleeps one interval, and the loop keeps running
        instead of dying."""
        ticks = {"n": 0}
        ...
        class OkWriter:
            async def write_batch(self, store, limit) -> Records:
                ticks["n"] += 1
                return Records([], 42, 0, 0)                # cursor set -> _tick calls store.drop
            async def teardown(self) -> None: ...

        class BoomStore(_FakeStore):
            async def drop(self, cursor: int) -> None:
                raise RuntimeError("disk hiccup")        # escapes _attempt: hits the loop guard
        ...
        task = asyncio.create_task(
            drain.drain(OkWriter(), BoomStore(), Upload(interval=7), Buffer(), clock)
        )
        while ticks["n"] < 3:
            await asyncio.sleep(0)
        task.cancel()
        with pytest.raises(asyncio.CancelledError):      # cancellation still propagates
            await task
        assert ticks["n"] >= 3                           # loop survived repeated tick failures
        assert clock.slept and set(clock.slept) == {7}   # each failure sleeps one interval
