...
def _manifest():
    return (
        ...
        .set_configuration({
            "snowflake": {"account": "acc", "user": "u", "warehouse": "WH", "database": "DB",
...
async def test_three_types_buffer_and_drain(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _reset(tmp_path)
    fake = FakeWriter()
    monkeypatch.setattr(main, "SnowflakeWriter", lambda cfg: fake)
...
async def test_invalid_configuration_raises(tmp_path: Path) -> None:
    ...
    bad = (
        ...
        .set_configuration({
            "snowflake": {"account": "acc", "user": "u", "warehouse": "WH", "database": "DB",
                          "schema": "PUBLIC", "table": "EVENTS", "auth": {"method": "password"}},
        })
        .build()
    )
