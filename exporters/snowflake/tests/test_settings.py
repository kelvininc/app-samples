...
class TestUploadAndBuffer:
    def test_defaults_and_bounds(self) -> None:
        ...
        assert s.buffer.max_backlog == 1_000_000       # bounded by default; 0 opts into unbounded

    def test_batch_size_accepts_the_bind_limit_cap(self) -> None:
        """4000 rows * 4 binds/row = 16,000 binds, just under Snowflake's ~16,384 ceiling."""
        assert Settings(**PW, upload={"batch_size": 4000}).upload.batch_size == 4000

    @pytest.mark.parametrize("bad", [{"batch_size": 0}, {"batch_size": 4001}, {"interval": 0}, {"interval": -1}])
    def test_rejects_out_of_bounds(self, bad: dict) -> None:
        """batch_size above 4000 is rejected at config time: the multi-row INSERT would
        exceed Snowflake's per-statement bind-variable limit and fail permanently."""
        with pytest.raises(ValidationError):
