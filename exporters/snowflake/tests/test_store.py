...
class TestAppend:
    ...
    async def test_aware_timestamp_round_trips_as_naive_utc(self, store: Store) -> None:
        """An aware timestamp is normalized to naive UTC before insert, so the stored wall
        time never depends on the host timezone (DuckDB would otherwise convert it to the
        session zone's wall time)."""
        aware = datetime(2026, 7, 1, 12, 0, tzinfo=timezone(timedelta(hours=5)))  # 07:00 UTC
        await store.append(aware, "pump-1", "temperature", 42.5)
        result = await store.read(10)
        ts = result.rows[0]["timestamp"]
        assert ts == datetime(2026, 7, 1, 7, 0)
